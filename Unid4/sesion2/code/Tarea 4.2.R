...
##############################################################################
###########################################
# Continuación: Clustering (muestras y sondas)
###########################################

# Paquetes (amap solo si quieres Dist con "pearson")
if (!requireNamespace("amap", quietly = TRUE)) install.packages("amap")
library(amap)

# =========================
# 1) Elegir set de sondas para clustering
# =========================
# Por defecto: usar sondas DE por interacción (más informativa en diseño 2x2)
de_col  <- "FDR.Int"      # puedes cambiar a "FDR.Geno" o "FDR.Trt"
fdr_use <- fdr_th         # 0.19

if (!de_col %in% colnames(results)) stop("No existe la columna: ", de_col)

keep <- results[[de_col]] <= fdr_use
cat("Sondas seleccionadas para clustering (", de_col, " <= ", fdr_use, "): ", sum(keep), "\n", sep="")

if (sum(keep) < 2) stop("Muy pocas sondas seleccionadas para clustering. Revisa 'de_col' o 'fdr_use'.")

# Matriz para clustering: genes/sondas (filas) x muestras (columnas)
X <- normdata[keep, , drop = FALSE]

# Para clustering de MUESTRAS con Euclidiana:
# estandariza por gen/sonda (fila), para igualar escala y varianza
Xz <- t(scale(t(X), center = TRUE, scale = TRUE))
Xz[is.na(Xz)] <- 0  # por si alguna fila quedó con varianza 0

# Guardar matriz usada
saveRDS(Xz, file = file.path(outdir, "matrix_for_clustering_Xz.rds"))

# =========================
# 2) Clustering jerárquico de MUESTRAS (Euclidiana + Ward.D2)
# =========================
d_samples  <- dist(t(Xz), method = "euclidean")
hc_samples <- hclust(d_samples, method = "ward.D2")

png(file.path(outdir, "hclust_samples_DE.png"), width = 8, height = 5, units = "in", res = 150)
par(mar=c(6,4,2,1), cex=.85)
plot(hc_samples,
     main = paste0("Hierarchical clustering (samples)\nEuclidean + Ward.D2 | ", de_col, "<=", fdr_use),
     xlab = "", sub = "", las = 2)
dev.off()

# =========================
# 3) Clustering jerárquico de SONDAS/GENES ((1 - Pearson) + Ward.D2)
# =========================
# Dist(..., method="pearson") en amap corresponde a distancia basada en correlación (≈ 1 - r)
# Aquí usamos X (sin z-score), porque Pearson ya compara perfiles y es invariante a escala/offset.
d_genes  <- Dist(X, method = "pearson")
hc_genes <- hclust(d_genes, method = "ward.D2")

png(file.path(outdir, "hclust_genes_DE.png"), width = 8, height = 5, units = "in", res = 150)
par(mar=c(2,4,2,1), cex=.65)
plot(hc_genes, labels = FALSE,
     main = paste0("Hierarchical clustering (genes)\n(1 - Pearson) + Ward.D2 | ", de_col, "<=", fdr_use),
     xlab = "", sub = "")
dev.off()

# =========================
# 4) SSQ (elbow) para elegir k (muestras y genes)
# =========================
# SSQ aproximada desde la matriz de distancias (cuadrados intra-cluster)
...
# Samples SSQ
D_samples   <- as.matrix(d_samples)
ssq_samples <- wss_from_hclust(hc_samples, D_samples, kmax = kmax)

png(file.path(outdir, "SSQ_samples.png"), width = 6, height = 4, units = "in", res = 150)
plot(1:kmax, ssq_samples, type = "b",
     xlab = "k (clusters)", ylab = "Within-cluster SSQ",
     main = "Elbow plot - samples")
dev.off()

# Genes SSQ (ojo: si hay muchas sondas puede ser pesado; en tu caso suele ser manejable)
D_genes   <- as.matrix(d_genes)
ssq_genes <- wss_from_hclust(hc_genes, D_genes, kmax = kmax)

png(file.path(outdir, "SSQ_genes.png"), width = 6, height = 4, units = "in", res = 150)
plot(1:kmax, ssq_genes, type = "b",
     xlab = "k (clusters)", ylab = "Within-cluster SSQ",
     main = "Elbow plot - genes")
dev.off()

# =========================
# 5) Elegir k + dendrogramas finales con rectángulos
# =========================
# Ajusta estos valores mirando SSQ_samples.png y SSQ_genes.png
k_samples <- 4
k_genes   <- 6

png(file.path(outdir, "hclust_samples_rect.png"), width = 8, height = 5, units = "in", res = 150)
par(mar=c(6,4,2,1), cex=.85)
plot(hc_samples, main = paste0("Samples clustering + rectangles (k=", k_samples, ")"),
     xlab = "", sub = "", las = 2)
rect.hclust(hc_samples, k = k_samples, border = "red")
dev.off()

png(file.path(outdir, "hclust_genes_rect.png"), width = 8, height = 5, units = "in", res = 150)
par(mar=c(2,4,2,1), cex=.65)
plot(hc_genes, labels = FALSE, main = paste0("Genes clustering + rectangles (k=", k_genes, ")"),
     xlab = "", sub = "")
rect.hclust(hc_genes, k = k_genes, border = "blue")
dev.off()

# =========================
# 6) Guardar asignación de clusters (para interpretación)
# =========================
cl_samples <- cutree(hc_samples, k = k_samples)
cl_genes   <- cutree(hc_genes,   k = k_genes)

write.csv(data.frame(Sample = names(cl_samples), Cluster = cl_samples),
          file.path(outdir, "clusters_samples.csv"), row.names = FALSE)

write.csv(data.frame(ProbeID = names(cl_genes), Cluster = cl_genes),
          file.path(outdir, "clusters_genes.csv"), row.names = FALSE)

cat("Listo: clustering + figuras + tablas guardadas en: ", outdir, "\n", sep = "")
